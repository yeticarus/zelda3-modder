...
static int ppu_getDbgPixelForBgLayer(Ppu *ppu, int x, int y, int layer);//DebugWindow || GuessBehindBlackBorders
static bool ppu_evaluateSpritesDbg(Ppu* ppu, int line, PpuZbufType* objBufferData);//DebugWindow || GuessBehindBlackBorders
...
void PpuBeginDrawing(Ppu *ppu, uint8_t *pixels, size_t pitch, uint32_t render_flags) {
  ...
  if (PpuGetCurrentRenderScale(ppu, ppu->renderFlags) == 4) {
    for (int i = 0; i < 256; i++) {
      ...
      ppu->colorMapRgb[i] = ppu->brightnessMult[color & 0x1f] << 16 |
...
void ppu_runLine(Ppu *ppu, int line) {
  if(line != 0) {
    ...
    if (line >= 225 + ppu->extraBottomCur) {
      memset(&ppu->renderBuffer[(line - 1) * ppu->renderPitch], 0, sizeof(uint32) * (256 + ppu->extraLeftRight * 2)); //(Why do we need this?)
      return;
    }
    ...
    if (ppu->renderFlags & kPpuRenderFlags_NewRenderer) {
      ...
      for (int x = 0; x < 256; x++) //GuessBehindBlackBorders: increasing limits here doesn't change much
        ppu_handlePixel(ppu, x, line);

      uint8 *dst = ppu->renderBuffer + ((line - 1) * ppu->renderPitch);
      if (ppu->extraLeftRight != 0) {
        memset(dst, 0, sizeof(uint32) * ppu->extraLeftRight);//(Why do we need this?)
        memset(dst + sizeof(uint32) * (256 + ppu->extraLeftRight), 0, sizeof(uint32) * ppu->extraLeftRight);//(Why do we need this?)
      }
    }
  }
}
...
#define READ_BITS(ta, tile) (addr = &ppu->vram[((ta) + (tile) * 16) & 0x7fff], addr[0] | addr[8] << 16)
  ...
  for (size_t windex = 0; windex < win.nr; windex++) {
    ...
    uint w = win.edges[windex + 1] - win.edges[windex];// GuessBehindBlackBorders (+100 doesn't change much)
...
static NOINLINE void PpuDrawWholeLine(Ppu *ppu, uint y) {
  if (ppu->forcedBlank) {
    ...
    memset(dst, 0, n); //Why do we need this?
    return;
  }
  ...
  uint32 windex = 0;
  do {
    uint32 left = cwin.edges[windex] + kPpuExtraLeftRight, right = cwin.edges[windex + 1] + kPpuExtraLeftRight; //GuessBehindBlackBorders: incrementing stuff here colors side bars in palette main color
    ...
    // Clear out stuff on the sides. // Or, instead, let's do a screen roll! (GuessBehindBlackBorders)
    if (ppu->extraLeftRight - ppu->extraLeftCur != 0){
        if(player_is_indoors || !g_config.guess_behind_black_borders ){
            // Set black bars on the left hand side:
...
void ppu_renderPartOfBgLayer(Ppu *ppu, int bg, uint offset, uint xstart, uint linestart, uint xmax) {
  ...
  for (int line = linestart; line < 64*8; line++) {
      if ((line-linestart) * ppu->renderPitch + offset < 64*8*8*8*8*8){ //  64*8*8*8*8*8 is rather empirical... I don't know the size of the renderBuffer /////
    uint32 *dst = (uint32*)&ppu->renderBuffer[(line-linestart) * ppu->renderPitch + offset];
    ...
    for (int x = xstart; x < IntMin(64*8,xmax); x++) {
      int pixel = 0;
      if (bg >= 0) {
...
//        )) {
//        color = 0xffff;//Tracé du rectangle blanc
//      }
//      if (bg < 0 && (
...
//        )) {
//        color = 0x059f; //Tracé du rectangle orange
//      }
...
//      }
      ...
      uint8 *color_map = ppu->brightnessMult;
      dst[0] = color_map[b] | color_map[g] << 8 | color_map[r] << 16; // here we assign the color to be to the pixel
  ...
  do {
    ...
    for (int col = 0; col < spriteSize; col += 8) {
      if (col + x > -8 - extra_left_right && col + x < 256 + extra_left_right) {
        ...
        // figure out which tile this uses, looping within 16x16 pages, and get its data
