...
#include "audio.h"
#include "assets.h"
#include "config.h" //GuessBehindBlackBorders
...
//  printf("main %d, sub %d  (%d, %d, %d)\n", main_module_index, submodule_index, BG2HOFS_copy2, room_bounds_x.v[2 | (quadrant_fullsize_x >> 1)], quadrant_fullsize_x >> 1);
  ...
  if (mod == 9) {
    if (main_module_index == 14 && submodule_index == 7 && overworld_map_state >= 4) {
      ...
      // outdoors
      extra_left = BG2HOFS_copy2 - ow_scroll_vars0.xstart;//GuessBehindBlackBorders: adding stuff here might restore the black border, not black but looped over background.
...
{ // fast way to do (alpha*p1+(255-alpha)*p2)/255
...
void ZeldaDrawPpuFrame(uint8 *pixel_buffer, size_t pitch, uint32 render_flags) {
  ...
  uint8_t ppu_extraLeftRight = g_zenv.ppu->extraLeftRight;
  uint8_t ppu_extraLeftCur =  g_zenv.ppu->extraLeftCur;
  uint8_t ppu_extraRightCur = g_zenv.ppu->extraRightCur;
  uint8_t ppu_extraBottomCur = g_zenv.ppu->extraBottomCur;
  uint32_t ppu_renderPitch = g_zenv.ppu->renderPitch;

  if (ppu_extraLeftRight != 0 || render_flags & kPpuRenderFlags_Height240)
  ...
  int bg = 1; //-1 for screen,
                //0 for fog, (canevas origin uncertain...)
                //1 (for sprites?) canevas can stick to background !
                //2 (for hud) canevas sticks to menu screen
  uint16_t vscroll =     g_zenv.ppu->bgLayer[bg].vScroll        %(64*8);
  uint16_t hscroll =    (g_zenv.ppu->bgLayer[bg].hScroll)       % (64*8);
  int xstart = 0;
  //Draw over black borders
  if(g_config.guess_behind_black_borders){
      if(!player_is_indoors){ //(indoors not supported)
          // If player near West border
          if (ppu_extraLeftRight - ppu_extraLeftCur != 0){
            ppu_renderPartOfBgLayer(g_zenv.ppu, //ppu
                               1, //bg (layer index)
                               ((-vscroll) * ppu_renderPitch + (2*ppu_extraLeftRight+256   -(ppu_extraLeftRight-ppu_extraLeftCur))*4), //offset
                               (ppu_extraLeftCur +256+ppu_extraLeftRight), //xstart
                               1,//linestart
                               64*8); //xmax
            // Deal with fog, rain, darkness,... (layer0) ?
            //ppu_renderPartOfBgLayer(g_zenv.ppu, //ppu
            //                   0, //bg (layer index)
            //                   ((-vscroll) * ppu_renderPitch + (2*ppu_extraLeftRight+256   -(ppu_extraLeftRight-ppu_extraLeftCur))*4), //offset
            //                   (ppu_extraLeftCur +256+ppu_extraLeftRight), //xstart
          ...
          // If player near East border
          if (ppu_extraLeftRight - ppu_extraRightCur != 0){
              // I haven't found the general formula (someone help me!), so here we go:
              if(ppu_extraLeftRight== 21){     //            4:3 aspect ratio
                ////no need
              }else if(ppu_extraLeftRight== 32){// extend_y, 4:3 aspect ratio
                ////no need
              }else if(ppu_extraLeftRight== 71){//           16:9 aspect ratio
                xstart = 64+50;
              }else if(ppu_extraLeftRight== 85){// extend_y, 16:9 aspect ratio
                xstart = 64+50 -28;
              }else if(ppu_extraLeftRight== 51){//           16:10 aspect ratio
                xstart = 64+50+40;
              }else if(ppu_extraLeftRight== 64){// extend_y, 16:10 aspect ratio
                xstart = 64+50+40 -26;
              }else if(ppu_extraLeftRight== 96){//           18:9 aspect ratio
                xstart = 64;
              }else if(ppu_extraLeftRight== 112){// extend_y, 18:9 aspect ratio
                xstart = 64 +28 +44; // <- Wild guess. I have actually no idea, because my game crashes in this config :-/
              }
              ppu_renderPartOfBgLayer(g_zenv.ppu, //ppu
                                 1, //bg (layer index)   (2 for hud, 1 for ground, 0 for...sprites?)
                                 ((-vscroll) * ppu_renderPitch ), //offset
                                 xstart, //xstart
                                 1, //linestart
                                 xstart + ppu_extraLeftRight-ppu_extraRightCur +1); //xmax

              // Deal with fog, rain, darkness,... (layer0) ?
                //ppu_renderPartOfBgLayer(g_zenv.ppu, //ppu
                //                 0, //bg (layer index)   (0 for fog, 1 for ground, 2 for hud,  -1 for sprites)
                //                 ((-vscroll) * ppu_renderPitch ), //offset
                //                 xstart, //xstart
                //                 1, //linestart
                //                 xstart + ppu_extraLeftRight-ppu_extraRightCur +1); //xmax
...
//ModOverLayer
  int x0 = 0 + hscroll ;
  int row0 = 0 +vscroll;
  int w = 256 +2*ppu_extraLeftRight;
  int h = 224 + g_zenv.ppu->extraBottomCur ;
  int index = 0;
  int highOam = g_zenv.ppu->oam[0x100 + (index >> 4)] >> (index & 15);
  int extramodx = (g_zenv.ppu->oam[index] & 0xff) - (highOam & 1) * 256;
  long rbuff_index = 0;
  uint16_t color  = 0x059f;
  uint32_t r = color & 0x1f;
  uint32_t g = (color >> 5) & 0x1f;
  uint32_t b = (color >> 10) & 0x1f;
  uint32_t alpha = 0;
  int nbNbColorChannel = 4;
  int link_x_onscreen = (link_x_coord +(ppu_extraLeftCur-ppu_extraRightCur)*(!player_is_indoors && g_config.guess_behind_black_borders )+ppu_extraLeftRight )%512;
  int link_y_onscreen = (link_y_coord +ppu_extraBottomCur*0 )%512; //TO BE TESTED

  for (int row = row0 ; row < row0+h ; row++){//reminder: int row = y - 1;
    for (int x = x0 ; x < x0+w ; x++){

        //note: 64*8*8*64*4 = 1048576
        // %((1024+g_zenv.ppu->extraLeftRight)*256);
        //Trivium: g_zenv.ppu->renderPitch == 4*w*4
        //int rbuff_index_max = ppu_renderPitch*h; // roughly ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *nbNbColorChannel *4 *h ?
        rbuff_index = ((row -vscroll) * (ppu_renderPitch)
                       + ((x -hscroll)
                       ) * nbNbColorChannel);

        //if (bg==0){
        //    // not appropriate use because on fog layer, but I didn't want to lose this nice tweeking for the moment:
        //    rbuff_index = ((row+ (-2*vscroll)) * ppu_renderPitch + (x -2*hscroll + ppu_extraLeftRight*0) * nbNbColorChannel)%(64*8*8*64*4);
        //}
        ...
        if( xx >=0 && yy>=0 && xx<MODOVERLAYERMAX_x && yy<MODOVERLAYERMAX_y){
            ...
            if(yy-512>0  && yy-512<MODOVERLAYERMAX_y){
                ...
                alpha = alpha<alphaModOverLayer[xx][yy-512]? alphaModOverLayer[xx][yy-512] : alpha;
            }
        }

         //Draw some extra green, as helpers
        //if(
        //        // horizontal line
        //        row==224
        ...
        //        || x == x0+w-1
        //        || row == row0+h -1
        //        || rbuff_index == ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *4
        //        || rbuff_index == ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *4-10
        //        || rbuff_index == ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *4+10
        //        || rbuff_index == ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *4-20
        //        || rbuff_index == ((g_zenv.ppu->extraLeftRight + 256 + g_zenv.ppu->extraLeftRight) * 224) *4+20

        //        //Little cross
        //        || rbuff_index == ((w) * 1) *nbNbColorChannel
        //        || rbuff_index == ((w) * 1) *nbNbColorChannel-8
        //        || rbuff_index == ((w) * 1 +w+ w +w ) *nbNbColorChannel+8
        //        || rbuff_index == ((w) * 1) *nbNbColorChannel-16
        //        || rbuff_index == ((w) * 1 +256 ) *nbNbColorChannel+16

        //        //Little square following link
        //        || (x >= (link_x_onscreen) && x<=(link_x_onscreen)+2  &&  row>=link_y_coord%512 && row<(link_y_coord)%512+2)
        //        || (x >= (link_x_onscreen+512) && x<=(link_x_onscreen+512)+2  &&  row>=link_y_coord%512 && row<(link_y_coord)%512+2)
        ...
        // Get the current renderBuffer, convert into uint32 to alpha blend with the modOverLayer
        uint32_t color_before = g_zenv.ppu->renderBuffer[2+rbuff_index]<<16 | g_zenv.ppu->renderBuffer[1+rbuff_index]<<8  | g_zenv.ppu->renderBuffer[0+rbuff_index];
        uint32_t color_mod = alpha<<24 | r<<16 | g<<8  | b ;
        uint32_t color_blended = AlphaBlendPixels(color_before, color_mod);

        //Extract individual int8 color channel from the int32 full color
        uint8_t a = (uint8_t)(color_blended >> 24);
        uint8_t r = (uint8_t)(color_blended >> 16);
        uint8_t g = (uint8_t)(color_blended >> 8);
        uint8_t b = (uint8_t)color_blended;

        // Assign to renderBuffer so that it displays the ModOverlayer later
        g_zenv.ppu->renderBuffer[0+rbuff_index] = b; //((b << 3) | (b >> 2)) * g_zenv.ppu->brightness / 15;
        g_zenv.ppu->renderBuffer[1+rbuff_index] = g; //((g << 3) | (g >> 2)) * g_zenv.ppu->brightness / 15;
        g_zenv.ppu->renderBuffer[2+rbuff_index] = r; // r/2 + g_zenv.ppu->renderBuffer[2+rbuff_index]/2;//((r << 3) | (r >> 2)) * g_zenv.ppu->brightness / 15;
        g_zenv.ppu->renderBuffer[3+rbuff_index] = 0;
    }
  }
