...
static bool HandleIniConfig(int section, const char *key, char *value) {
  if (section == 0) {//[keyMap]
    ...
    if (StringEqualsNoCase(key, "Autosave")) {
      ...
      return true;
    }else if (StringEqualsNoCase(key, "DebugWindow")) { //DebugWindow
        return ParseBool(value, &g_config.debug_window); //DebugWindow

    }else if (StringEqualsNoCase(key, "GuessBehindBlackBorders")) { //GuessBehindBlackBorders
        return ParseBool(value, &g_config.guess_behind_black_borders); //GuessBehindBlackBorders
    }
    ...
    } else if (StringEqualsNoCase(key, "ExtraBumps")) {
        return ParseBoolBit(value, &g_config.features0, kFeatures0_ExtraBumps);
    }
  }
  return false;
}
